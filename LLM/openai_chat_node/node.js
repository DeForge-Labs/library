...
class openai_chat_node extends BaseNode {
  ...
  async initializeVectorStore(embeddings, tableName, webconsole) {
    try {
      ...
      webconsole.error(
        `OPENAI NODE | Failed to initialize PostgreSQL vector store '${tableName}': ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async initializeChatHistory(sessionId, webconsole) {
    try {
      ...
      webconsole.success(
        `OPENAI NODE | PostgreSQL chat history initialized for session: ${sessionId}`,
      );
      return chatHistory;
    } catch (error) {
      webconsole.error(
        `OPENAI NODE | Failed to initialize chat history: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  createRetriever(vectorStore, webconsole) {
    ...
    webconsole.info(
      "OPENAI NODE | Created PostgreSQL-based retriever with k=5",
    );
    return retriever;
  }
    ...
    return tool(
      async ({ query }, toolConfig) => {
        try {
          ...
          webconsole.info(
            `OPENAI NODE | Retrieved ${docs.length} relevant documents from PostgreSQL`,
          );
  ...
  async loadChatHistory(sessionId, webconsole) {
    try {
      ...
      webconsole.info(
        `OPENAI NODE | Loaded ${messages.length} messages from chat history`,
      );
      return messages;
    } catch (error) {
      webconsole.error(
        `OPENAI NODE | Error loading chat history: ${error.message}`,
      );
      return [];
    }
  }
    ...
    try {
      ...
      const query =
        queryFilter.length > 0
          ? queryFilter[0].value
          : content.find((e) => e.name === "Query")?.value || "";
      ...
      webconsole.error(
        "OPENAI NODE | Falling back to min required credits | Error estimating usage: ",
        error,
      );
  ...
  async run(inputs, contents, webconsole, serverData) {
    try {
      ...
      const model = contents.find((e) => e.name === "Model")?.value || "gpt-4o";
      const modelMap = {
        "gpt-5": "openai/gpt-5",
        "gpt-5.3": "openai/gpt-5.3-chat",
        "gpt-5-mini": "openai/gpt-5-mini",
      ...
      webconsole.info(
        `OPENAI NODE | Configuration - Model: ${model}, Temperature: ${temperature}, Save Memory: ${saveMemory}, RAG Table: ${ragTableName || "None"}`,
      );
      ...
      for (const fileLink of files) {
        ...
        if (fileCount > 5) {
          webconsole.warn(
            "OPENAI NODE | Maximum of 5 files are allowed, skipping remaining files",
          );
          break;
        }
        try {
          ...
          const downloader = new Downloader({
            ...
            onResponse: (response) => {
              ...
              if (contentLength && parseInt(contentLength) > 25 * 1024 * 1024) {
                webconsole.error(
                  `OPENAI NODE | File at ${fileLink} exceeds the 25 MB size limit. Skipping this file.`,
                );
                skipFile = true;
                return false;
              }
              ...
              if (
                ...
                webconsole.error(
                  `OPENAI NODE | File at ${fileLink} is not an image or PDF (content-type: ${contentType}). Skipping this file. If you believe this is an error, please upload the file to some other service`,
                );
                skipFile = true;
                return false;
              }
          ...
          webconsole.error(
            "OPENAI NODE | Some problem occured parsing file, skipping: ",
            error,
          );
        }
      }
      ...
      if (fileObjs.length > 0) {
        webconsole.info(
          `OPENAI NODE | Attaching ${fileObjs.length} files to the prompt`,
        );
        inputMessages.push(
          new HumanMessage({
            content: [
      ...
      if (saveMemory) {
        try {
          ...
          webconsole.error(
            `OPENAI NODE | Error saving chat history: ${error.message}`,
          );
        }
      }

      webconsole.success(
        "OPENAI NODE | Successfully generated response with LangGraph",
      );
      return {
