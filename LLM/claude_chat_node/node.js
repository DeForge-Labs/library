...
class claude_chat_node extends BaseNode {
  ...
  async initializeVectorStore(embeddings, tableName, webconsole) {
    try {
      ...
      webconsole.error(
        `CLAUDE NODE | Failed to initialize PostgreSQL vector store '${tableName}': ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async initializeChatHistory(sessionId, webconsole) {
    try {
      ...
      webconsole.success(
        `CLAUDE NODE | PostgreSQL chat history initialized for session: ${sessionId}`,
      );
      return chatHistory;
    } catch (error) {
      webconsole.error(
        `CLAUDE NODE | Failed to initialize chat history: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  createRetriever(vectorStore, webconsole) {
    ...
    webconsole.info(
      "CLAUDE NODE | Created PostgreSQL-based retriever with k=5",
    );
    return retriever;
  }
    ...
    return tool(
      async ({ query }, toolConfig) => {
        try {
          ...
          webconsole.info(
            `CLAUDE NODE | Retrieved ${docs.length} relevant documents from PostgreSQL`,
          );
  ...
  async loadChatHistory(sessionId, webconsole) {
    try {
      ...
      webconsole.info(
        `CLAUDE NODE | Loaded ${messages.length} messages from chat history`,
      );
      return messages;
    } catch (error) {
      webconsole.error(
        `CLAUDE NODE | Error loading chat history: ${error.message}`,
      );
      return [];
    }
  }
  ...
  async estimateUsage(inputs, contents, serverData) {
    try {
      ...
      const model =
        contents.find((e) => e.name === "Model")?.value ||
        "claude-3-7-sonnet-latest";
      ...
      const { message, inputTokenCostPerToken, outputTokenCostPerToken } =
        await serverData.openrouterUtil.getModelPricing(modelMap[model]);
      ...
      console.error(
        `CLAUDE NODE | Falling back to default value | Error estimating usage: ${error.message}`,
      );
  ...
  async run(inputs, contents, webconsole, serverData) {
    try {
      ...
      webconsole.info(
        `CLAUDE NODE | Configuration - Model: ${model}, Temperature: ${temperature}, Save Memory: ${saveMemory}, RAG Table: ${ragTableName || "None"}`,
      );
      ...
      for (const fileLink of files) {
        ...
        if (fileCount > 5) {
          webconsole.warn(
            "LAUDENODE | Maximum of 5 files are allowed, skipping remaining files",
          );
          break;
        }
        try {
          ...
          const downloader = new Downloader({
            ...
            onResponse: (response) => {
              ...
              if (contentLength && parseInt(contentLength) > 25 * 1024 * 1024) {
                webconsole.error(
                  `LAUDENODE | File at ${fileLink} exceeds the 25 MB size limit. Skipping this file.`,
                );
                skipFile = true;
                return false;
              }
              ...
              if (
                ...
                webconsole.error(
                  `LAUDENODE | File at ${fileLink} is not an image or PDF (content-type: ${contentType}). Skipping this file. If you believe this is an error, please upload the file to some other service`,
                );
                skipFile = true;
                return false;
              }
          ...
          webconsole.error(
            "LAUDENODE | Some problem occured parsing file, skipping: ",
            error,
          );
        }
      }
      ...
      if (fileObjs.length > 0) {
        webconsole.info(
          `LAUDENODE | Attaching ${fileObjs.length} files to the prompt`,
        );
        inputMessages.push(
          new HumanMessage({
            content: [
      ...
      const totalInputCost = Math.ceil(
        inputTokenUsage * inputTokenCostPerToken,
      );
      const totalOutputCost = Math.ceil(
        outputTokenUsage * outputTokenCostPerToken,
      );
      ...
      if (saveMemory) {
        try {
          ...
          webconsole.error(
            `CLAUDE NODE | Error saving chat history: ${error.message}`,
          );
        }
      }

      webconsole.success(
        "CLAUDE NODE | Successfully generated response with LangGraph",
      );
      return {
