...
class google_chat_node extends BaseNode {
  ...
  async initializeVectorStore(embeddings, tableName, webconsole) {
    try {
      ...
      webconsole.error(
        `GOOGLE NODE | Failed to initialize PostgreSQL vector store '${tableName}': ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async initializeChatHistory(sessionId, webconsole) {
    try {
      ...
      webconsole.success(
        `GOOGLE NODE | PostgreSQL chat history initialized for session: ${sessionId}`,
      );
      return chatHistory;
    } catch (error) {
      webconsole.error(
        `GOOGLE NODE | Failed to initialize chat history: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  createRetriever(vectorStore, webconsole) {
    ...
    webconsole.info(
      "GOOGLE NODE | Created PostgreSQL-based retriever with k=5",
    );
    return retriever;
  }
    ...
    return tool(
      async ({ query }, toolConfig) => {
        try {
          ...
          webconsole.info(
            `GOOGLE NODE | Retrieved ${docs.length} relevant documents from PostgreSQL`,
          );
  ...
  async loadChatHistory(sessionId, webconsole) {
    try {
      const chatHistory = await this.initializeChatHistory(
        sessionId,
        webconsole,
      );
      ...
      webconsole.info(
        `GOOGLE NODE | Loaded ${messages.length} messages from chat history`,
      );
      return messages;
    } catch (error) {
      webconsole.error(
        `GOOGLE NODE | Error loading chat history: ${error.message}`,
      );
      return [];
    }
  }
  ...
  createWorkflow(llm, systemPrompt, tools, webconsole) {
    ...
    const callModel = async (state, config) => {
      try {
        ...
        const lastHumanMesageIndex = reversedMessages.findIndex(
          (msg) => msg.getType() === "human",
        );

        if (lastHumanMesageIndex > 0) {
          const lastTurnMessages = reversedMessages.slice(
            0,
            lastHumanMesageIndex,
          );

          const toolMessages = lastTurnMessages.filter(
    ...
    try {
      ...
      const query =
        queryFilter.length > 0
          ? queryFilter[0].value
          : contents.filter((e) => e.name === "Query")[0].value || "";

      const model =
        contents.find((e) => e.name === "Model")?.value || "gemini-2.0-flash";
      ...
      const inputPrice =
        queryTokens > 200000
          ? modelPricingInputOver200[model]
          : modelPricingInputUnder200[model];

      return Math.ceil(queryTokens * (inputPrice / 1e6));
    } catch (error) {
      webconsole.error(
        `GOOGLE NODE | Fallback to default value | Error estimating usage: ${error.message}`,
      );
  ...
  async run(inputs, contents, webconsole, serverData) {
    try {
      ...
      const systemPromptFilter = inputs.filter(
        (e) => e.name === "System Prompt",
      );
      let systemPrompt =
        systemPromptFilter.length > 0
          ? systemPromptFilter[0].value
          : contents.filter((e) => e.name === "System Prompt")[0].value ||
      ...
      if (tools.length > 0) {
        webconsole.info(
          "GOOGLE NODE | Generating tool descriptions for system prompt",
        );
      ...
      let temperature =
        temperatureFilter.length > 0
          ? temperatureFilter[0].value
          : contents.filter((e) => e.name === "Temperature")[0].value || 0.3;
      ...
      const saveMemory =
        contents.find((e) => e.name === "Save Context")?.value || false;

      const ragStoreFilter = inputs.filter((e) => e.name === "RAG");
      const ragTableName =
        ragStoreFilter.length > 0 ? ragStoreFilter[0].value : "";

      webconsole.info(
        `GOOGLE NODE | Configuration - Model: ${model}, Temperature: ${temperature}, Save Memory: ${saveMemory}, RAG Table: ${ragTableName || "None"}`,
      );
      ...
      if (ragTableName && ragTableName.trim() !== "") {
        ...
        if (!process.env.OPENAI_API_KEY) {
          throw new Error(
            "OPENAI_API_KEY environment variable required for RAG embeddings",
          );
        }
      ...
      for (const fileLink of files) {
        ...
        if (fileCount > 5) {
          webconsole.warn(
            "GOOGLE NODE | Maximum of 5 files are allowed, skipping remaining files",
          );
          break;
        }
        try {
          ...
          const downloader = new Downloader({
            ...
            onResponse: (response) => {
              ...
              if (contentLength && parseInt(contentLength) > 50 * 1024 * 1024) {
                webconsole.error(
                  `GOOGLE NODE | File at ${fileLink} exceeds the 25 MB size limit. Skipping this file.`,
                );
                skipFile = true;
                return false;
              }
              ...
              if (
                ...
                webconsole.error(
                  `GOOGLE NODE | File at ${fileLink} is not an image or PDF (content-type: ${contentType}). Skipping this file. If you believe this is an error, please upload the file to some other service`,
                );
                skipFile = true;
                return false;
              }
          ...
          webconsole.error(
            "GOOGLE NODE | Some problem occured parsing file, skipping: ",
            error,
          );
        }
      }
      ...
      if (fileObjs.length > 0) {
        webconsole.info(
          `GOOGLE NODE | Attaching ${fileObjs.length} files to the prompt`,
        );
        inputMessages.push(
          new HumanMessage({
            content: [
      ...
      const thisTurnMessages = finalState.values.messages.slice(
        pastMessages.length,
      );
      ...
      const inputCreditRate =
        inputTokenUsage > 200000
          ? modelPricingInputOver200[model]
          : modelPricingInputUnder200[model];
      const outputCreditRate =
        outputTokenUsage > 200000
          ? modelPricingOutputOver200[model]
          : modelPricingOutputUnder200[model];

      const inputCreditUsage = Math.ceil(
        inputTokenUsage * (inputCreditRate / 1e6),
      );
      const outputCreditUsage = Math.ceil(
        outputTokenUsage * (outputCreditRate / 1e6),
      );
      ...
      if (saveMemory) {
        try {
          ...
          webconsole.error(
            `GOOGLE NODE | Error saving chat history: ${error.message}`,
          );
        }
      }

      webconsole.success(
        "GOOGLE NODE | Successfully generated response with LangGraph",
      );
      return {
