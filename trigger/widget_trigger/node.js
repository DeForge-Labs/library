...
class widget_trigger extends BaseNode {
  ...
  async run(inputs, contents, webconsole, serverData) {
    try {
      ...
      const Intro =
        IntroFilter?.value ||
        contents.find((e) => e.name === "Intro")?.value ||
      ...
      const CompanyName =
        ...
        contents.find((e) => e.name === "Company Name")?.value ||
        "Deforge Assistant";
      ...
      const Logo =
        LogoFilter?.value ||
        contents.find((e) => e.name === "Logo")?.value ||
        "";
      ...
      if (initReq) {
        webconsole.success(
          "WIDGET NODE | Recieved init request, responded successfully",
        );
        return {
          Flow: false,
          Message: "",
