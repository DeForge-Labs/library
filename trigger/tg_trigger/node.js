...
class tg_trigger extends BaseNode {
  ...
  async run(inputs, contents, webconsole, serverData) {
    try {
      ...
      let onStartText =
        onStartFilter.length > 0
          ? onStartFilter[0].value
          : contents.filter((e) => e.name === "On Start")[0].value || "";
      onStartText =
        onStartText.length > 4096
          ? onStartText.slice(0, -3) + "..."
          : onStartText;
      ...
      if (isCommand && msg.startsWith("/start")) {
        const startResponse = await axios.get(
          `https://api.telegram.org/bot${botToken}/sendMessage?chat_id=${chatID}&text=${onStartText}`,
        );

        if (startResponse.data.ok) {
          webconsole.success(
            "TG NODE | Recieved start message, responded successfully",
          );
          return {
            Flow: false,
            Message: msg,
      ...
      let voiceFileURL = "";
      if (voice) {
        const getFileURL = await axios.get(
          `https://api.telegram.org/bot${botToken}/getFile?file_id=${voice}`,
        );
        if (getFileURL.data.ok) {
          ...
          if (!fileType || !fileType.mime.startsWith("audio/")) {
            webconsole.error(
              "TG NODE | The downloaded file is not a valid audio file.",
            );
